...
/* 이미 끝난 학기인지 판단 — 지난 학기는 데이터가 변하지 않으므로 영구 캐시 사용
   끝났다고 보는 기준:
     - 요청 연도 < 현재 연도: 모든 학기
     - 요청 연도 == 현재 연도:
         1학기(term=1): 7월 이후 (종강 기준)
         여름학기(term=3): 9월 이후
         2학기·겨울학기: 같은 연도 내에는 아직 끝나지 않음 */
function isPastSemester(year, term) {
  const now  = new Date();
  const nowY = now.getFullYear();
  const nowM = now.getMonth() + 1;
  const yr   = Number(year);
  if (yr < nowY) return true;
  if (yr > nowY) return false;
  const endMonth = { '1': 7, '3': 9 }; // 2·4학기는 같은 연도 내 종강 없음
  return nowM >= (endMonth[String(term)] ?? Infinity);
}

/* 캐시 파일이 있으면 항상 즉시 반환 — TTL 없음 (학기 데이터는 바뀌지 않음) */
  ...
  if (pathname === '/api/courses') {
    ...
    if (frozen) {
      if (fs.existsSync(cachePath)) {
        console.log(`[server] 지난 학기 캐시 반환 (영구): ${cachePath}`);
        return sendCache(res, cachePath, year, sem, false);
      }
      // 캐시 파일이 아직 없는 경우 → 1회 크롤 후 영구 저장
      ...
      console.log(`[server] ${year}-${sem} 지난 학기 최초 크롤링 시작`);
      ...
      crawlCourses(year, term).then(courses => {
        ...
        console.log(`[server] 지난 학기 캐시 저장 (영구): ${cachePath}`);
      }).catch(e => console.error('[server] 크롤링 실패:', e.message))
        .finally(() => crawlingInProgress.delete(cacheKey));
      return;
    }

    // ── 케이스 A: 진행 중/예정 학기 (개설 공시됨)
    if (published) {
      // 캐시가 있으면 무조건 즉시 반환 (TTL 없음 — 학기 데이터는 변하지 않음)
      if (fs.existsSync(cachePath)) {
        return sendCache(res, cachePath, year, sem, false);
      }

      // 크롤 진행 중이면 완료 대기 후 반환
      if (crawlingInProgress.has(cacheKey)) {
        let waited = 0;
        while (crawlingInProgress.has(cacheKey) && waited < 120000) {
          await sleep(1000); waited += 1000;
        }
        if (fs.existsSync(cachePath)) return sendCache(res, cachePath, year, sem, false);
        res.writeHead(503); res.end(JSON.stringify({error:'크롤링 타임아웃'}));
      ...
      // 캐시 없음 → 크롤링 시작 (202 반환, 클라이언트가 폴링)
      crawlingInProgress.add(cacheKey);
      console.log(`[server] ${year}-${sem} 첫 크롤링 시작`);
      res.writeHead(202, {'Content-Type':'application/json', 'X-Is-Fallback':'false'});
      res.end(JSON.stringify({status:'crawling', message:`${year} ${sem} 강의 데이터 수집 중… 잠시만 기다려 주세요.`}));
      crawlCourses(year, term).then(courses => {
        if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, {recursive:true});
        fs.writeFileSync(cachePath, JSON.stringify(courses, null, 2), 'utf-8');
        console.log(`[server] 캐시 저장 완료: ${cachePath}`);
      }).catch(e => console.error('[server] 크롤링 실패:', e.message))
        .finally(() => crawlingInProgress.delete(cacheKey));
      return;
    }
...
server.listen(PORT, () => {
  ...
  // 서버 시작 시 현재 학기 캐시 없으면 즉시 백그라운드 크롤
  // → 첫 방문자가 로딩을 기다리지 않도록 미리 준비
  (() => {
    const now   = new Date();
    ...
    // 현재 달 기준 진행 중인 학기 추정
    let term;
    if      (month >= 9)  term = '2'; // 2학기
    else if (month >= 7)  term = '3'; // 여름학기
    else if (month >= 3)  term = '1'; // 1학기
    else                  term = '4'; // 겨울학기 (1~2월)

    const cacheKey  = `courses_${year}_${term}`;
    const cachePath = path.join(DATA_DIR, `${cacheKey}.json`);
    if (!fs.existsSync(cachePath) && !crawlingInProgress.has(cacheKey)) {
      const sem = termToSemester(term);
      console.log(`[server] 시작 시 ${year} ${sem} 선크롤 시작…`);
      crawlingInProgress.add(cacheKey);
      crawlCourses(year, term).then(courses => {
        if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, {recursive:true});
        fs.writeFileSync(cachePath, JSON.stringify(courses, null, 2), 'utf-8');
        console.log(`[server] 선크롤 완료: ${cachePath}`);
      }).catch(e => console.error('[server] 선크롤 실패:', e.message))
        .finally(() => crawlingInProgress.delete(cacheKey));
    }
  })();
});
